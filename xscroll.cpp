...
#if wxUSE_TIMER
// ----------------------------------------------------------------------------
// xAutoScrollTimer: the timer used to generate a stream of scroll events when
// a captured mouse is held outside the window
// ----------------------------------------------------------------------------

class xAutoScrollTimer : public wxTimer
{
public:
    xAutoScrollTimer(wxWindow *winToScroll, xScrollHelper *scroll,
...
private:
    ...
    DECLARE_NO_COPY_CLASS(xAutoScrollTimer)
};
...
xAutoScrollTimer::xAutoScrollTimer(wxWindow *winToScroll,
                                     xScrollHelper *scroll,
                                     wxEventType eventTypeToSend,
                                     int pos, int orient)
{
    m_win = winToScroll;
...
bool xScrollHelperEvtHandler::ProcessEvent(wxEvent& event)
{
...
// ----------------------------------------------------------------------------
// xScrollHelper construction
// ----------------------------------------------------------------------------

xScrollHelper::xScrollHelper(wxWindow *win)
{
...
void xScrollHelper::SetScrollbars(int pixelsPerUnitX,
                                   int pixelsPerUnitY,
                                   int noUnitsX,
                                   int noUnitsY,
                                   int xPos,
...
void xScrollHelper::DeleteEvtHandler()
{
...
void xScrollHelper::SetWindow(wxWindow *win)
{
    wxCHECK_RET( win, _T("xScrollHelper needs a window to scroll") );
...
void xScrollHelper::DoSetTargetWindow(wxWindow *target)
{
    ...
    if ( m_targetWindow == m_win )
    {
        ...
        m_handler = new xScrollHelperEvtHandler(this);
        m_targetWindow->PushEventHandler(m_handler);
    }
}

void xScrollHelper::SetTargetWindow(wxWindow *target)
{
...
wxWindow *xScrollHelper::GetTargetWindow() const
{
...
void xScrollHelper::HandleOnScroll(wxScrollWinEvent& event)
{
...
int xScrollHelper::CalcScrollInc(wxScrollWinEvent& event)
{
...
void xScrollHelper::AdjustScrollbars()
{
...
void xScrollHelper::DoPrepareDC(wxDC& dc)
{
...
void xScrollHelper::SetScrollRate( int xstep, int ystep )
{
...
void xScrollHelper::GetScrollPixelsPerUnit (int *x_unit, int *y_unit) const
{
    if ( x_unit )
...
int xScrollHelper::GetScrollPageSize(int orient) const
{
    if ( orient == wxHORIZONTAL )
...
void xScrollHelper::SetScrollPageSize(int orient, int pageSize)
{
    if ( orient == wxHORIZONTAL )
 ...
 */
void xScrollHelper::Scroll( int x_pos, int y_pos )
{
    if (!m_targetWindow)
        return;
...
void xScrollHelper::EnableScrolling (bool x_scroll, bool y_scroll)
{
...
void xScrollHelper::GetViewStart (int *x, int *y) const
{
    if ( x )
        *x = m_xScrollPosition;
    if ( y )
...
void xScrollHelper::ViewStart(int *x, int *y) const
{
    GetViewStart( x, y );
}
...
void xScrollHelper::DoCalcScrolledPosition(int x, int y, int *xx, int *yy) const
{
    if ( xx )
...
void xScrollHelper::DoCalcUnscrolledPosition(int x, int y, int *xx, int *yy) const
{
    if ( xx )
        *xx = x + m_xScrollPosition * m_xScrollPixelsPerLine;
    if ( yy )
...
void xScrollHelper::HandleOnSize(wxSizeEvent& WXUNUSED(event))
{
...
// scroll position
void xScrollHelper::HandleOnPaint(wxPaintEvent& WXUNUSED(event))
{
...
void xScrollHelper::HandleOnChar(wxKeyEvent& event)
{
...
bool xScrollHelper::SendAutoScrollEvents(wxScrollWinEvent& event) const
{
...
void xScrollHelper::StopAutoScrolling()
{
#if wxUSE_TIMER
...
void xScrollHelper::HandleOnMouseEnter(wxMouseEvent& event)
{
    StopAutoScrolling();
...
void xScrollHelper::HandleOnMouseLeave(wxMouseEvent& event)
{
...
#if wxUSE_TIMER
        delete m_timerAutoScroll;
        m_timerAutoScroll = new xAutoScrollTimer
                                (
                                    m_targetWindow, this,
...
void xScrollHelper::HandleOnMouseWheel(wxMouseEvent& event)
{
